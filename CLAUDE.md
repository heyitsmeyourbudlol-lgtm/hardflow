...
| `gizmos/tools.py` | Workspace Tools (toolbar T): `HARDFLOW_T_move/rotate/scale/bevel` + `push_pull` (Object, launches the raycast modal) + `push_pull_edit` (Edit Mesh, gizmo); `register`/`unregister` wrap `register_tool` defensively (it can raise headless). The `after` anchor is `builtin.transform` and is gated by `_anchor_is_standalone` (via `_toolbar_items`): `register_tool` splices an `after` match into the *group tuple* it was found in, so anchoring on a grouped id (`builtin.scale` lives in the `(scale, scale_cage)` group) buried the tools in that dropdown and, with `separator=True`, wrote a bare `None` inside the group -- which Blender's own `_tool_get_group_by_id` walks unguarded, raising `AttributeError: 'NoneType' object has no attribute 'idname'` on every toolbar redraw (issue #4). A grouped/missing anchor now degrades to appending |
